...
@Component({
  selector: "app-save-update-proveedores-modal",
  templateUrl: "./save-update-proveedores-modal.component.html",
  styleUrls: ["./save-update-proveedores-modal.component.scss"],
  providers: [
    DatePipe,
...
export class SaveUpdateProveedoresModalComponent implements OnInit {
  ...
  getDataProveedor(Proveedor) {
    this.proveedor_s.GetDatosProveedor(Proveedor).subscribe(
      (data: any) => {
        ...
        this.formDatosPersonales.controls.RazonSocial.setValue(
          dataGenerales.razonSocial
        );
        this.formDatosPersonales.controls.NombreComercial.setValue(
          dataGenerales.nombreComercial
        );
        this.formDatosPersonales.controls.Direccion.setValue(
          dataGenerales.direccion
        );
        this.formDatosPersonales.controls.Referencia.setValue(
          dataGenerales.referencia
        );
        this.formDatosPersonales.controls.DocumentoIdentidad.setValue(
          dataGenerales.documentoIdentidad
        );
        this.formDatosPersonales.controls.Telefono.setValue(
          dataGenerales.telefono1
        );
        this.formDatosPersonales.controls.Telefono2.setValue(
          dataGenerales.telefono2
        );
        ...
        if (dataGenerales.idPais !== 1) {
          ...
          this.formDatosPersonales.addControl(
            "Departamento",
  ...
  llenarContactos(Contactos) {
    for (let i = 0; i <= Contactos.length - 1; i++) {
      ...
      this.deta_formTipoDocumentoContacto.push(
        new FormControl(Contactos[i].idTipoDocumentoIdentidad, [
          Validators.required,
        ])
      );
      this.deta_formDocumentoIdentidadContacto.push(
        new FormControl(Contactos[i].documentoIdentidad, [Validators.required])
      );
      this.deta_formNombre.push(
        new FormControl(Contactos[i].nombres, [Validators.required])
      );
      this.deta_formApellidoPaterno.push(
        new FormControl(Contactos[i].apePaterno, [Validators.required])
      );
      this.deta_formApellidoMaterno.push(
        new FormControl(Contactos[i].apeMaterno, [Validators.required])
      );
      this.deta_formCargo.push(
        new FormControl(Contactos[i].cargo, [Validators.required])
      );
      this.deta_formTelefonoContacto.push(
        new FormControl(Contactos[i].telefono, [Validators.required])
      );
      this.deta_formEmailContacto.push(
        new FormControl(Contactos[i].email, [Validators.required])
      );
  ...
  llenarCuentasBancarias(CuentasBancarias) {
    for (let i = 0; i <= CuentasBancarias.length - 1; i++) {
      ...
      this.deta_formBanco.push(
        new FormControl(CuentasBancarias[i].idBanco, [Validators.required])
      );
      this.deta_formTitular.push(
        new FormControl(CuentasBancarias[i].titular, [Validators.required])
      );
      this.deta_formTipoCuenta.push(
        new FormControl(CuentasBancarias[i].idTipoCuentaBancaria, [
          Validators.required,
        ])
      );
      this.deta_formTipoMoneda.push(
        new FormControl(CuentasBancarias[i].idMoneda, [Validators.required])
      );
      this.deta_formNumeroCuenta.push(
        new FormControl(CuentasBancarias[i].nroCuenta, [Validators.required])
      );
      this.deta_formCuentaInterbancaria.push(
        new FormControl(CuentasBancarias[i].nroCCI, [Validators.required])
      );
      this.array_cuentasBancarias[i].Detraccion =
        CuentasBancarias[i].detraccion;
      ...
      (data: any) => {
        if (dataCabecera != null) {
          if (
            data.find(
              (categoria) =>
                categoria.idCategoriaProveedor ===
                dataCabecera.idCategoriaProveedor
            ) === undefined
          ) {
            data.push({
          ...
          this.array_categorias = data.sort((a, b) =>
            a.NombreCategoria > b.NombreCategoria ? 1 : -1
          );
          this.formDatosPersonales.controls.Categoria.setValue(
            dataCabecera.idCategoriaProveedor
          );
        } else {
          this.array_categorias = data.sort((a, b) =>
      ...
      (data: any) => {
        if (dataCabecera != null) {
          if (dataCabecera.idPais != null) {
            if (
              data.find((pais) => pais.Pais === dataCabecera.idPais) ===
              undefined
            ) {
              data.push({
            ...
            this.formDatosPersonales.controls.Pais.setValue(
              dataCabecera.idPais
            );
          }
        } else {
  ...
  getListarDepartamento(Pais, dataCabecera) {
    if (Pais !== 1) {
      ...
      this.formDatosPersonales.controls.Departamento.setValidators(
        Validators.compose([Validators.required])
      );
      this.formDatosPersonales.controls.Provincia.setValidators(
        Validators.compose([Validators.required])
      );
      this.formDatosPersonales.controls.Distrito.setValidators(
        Validators.compose([Validators.required])
      );
    }
        ...
        if (dataCabecera != null) {
          if (dataCabecera.idDepartamento != null) {
            if (
              data.find(
                (departamento) =>
                  departamento.Departamento === dataCabecera.idDepartamento
              ) === undefined
            ) {
              data.push({
            ...
            this.array_departamento = data.sort((a, b) =>
              a.NombreDepartamento > b.NombreDepartamento ? 1 : -1
            );
            this.formDatosPersonales.controls.Departamento.setValue(
              dataCabecera.idDepartamento
            );
          }
        } else {
          this.array_departamento = data.sort((a, b) =>
        ...
        if (dataCabecera != null) {
          if (dataCabecera.idProvincia != null) {
            if (
              data.find(
                (provincia) =>
                  provincia.NombreProvincia === dataCabecera.idProvincia
              ) === undefined
            ) {
              data.push({
            ...
            this.array_provincia = data.sort((a, b) =>
              a.NombreProvincia > b.NombreProvincia ? 1 : -1
            );
            this.formDatosPersonales.controls.Provincia.setValue(
              dataCabecera.idProvincia
            );
          }
        } else {
          this.array_provincia = data.sort((a, b) =>
        ...
        if (dataCabecera != null) {
          if (dataCabecera.idDistrito != null) {
            if (
              data.find(
                (distrito) =>
                  distrito.NombreDistrito === dataCabecera.idDistrito
              ) === undefined
            ) {
              data.push({
            ...
            this.array_distrito = data.sort((a, b) =>
              a.NombreDistrito > b.NombreDistrito ? 1 : -1
            );
            this.formDatosPersonales.controls.Distrito.setValue(
              dataCabecera.idDistrito
            );
          }
        } else {
          this.array_distrito = data.sort((a, b) =>
      ...
      (data: any) => {
        if (dataCabecera != null) {
          if (
            data.find(
              (tipoDocIdentidad) =>
                tipoDocIdentidad.ValorDocIdentidad ===
                dataCabecera.idTipoDocumentoIdentidad
            ) === undefined
          ) {
            data.push({
          ...
          this.formDatosPersonales.controls.TipoDocumento.setValue(
            dataCabecera.idTipoDocumentoIdentidad
          );
        } else {
          this.array_tipo_doc = data;
  ...
  prepare_model() {
    ...
    for (let i = 0; i < this.array_contacto_eliminado.length; i++) {
      datoContacto.push({
        idProveedor: this.idProveedor,
        idProveedorContacto:
          this.array_contacto_eliminado[i].idProveedorContacto,
        ...
        idTipoDocumentoIdentidad:
          this.array_contacto_eliminado[i].TipoDocumentoContacto,
        documentoIdentidad:
          this.array_contacto_eliminado[i].DocumentoIdentidadContacto,
    ...
    for (let i = 0; i < this.array_cuentasBancarias.length; i++) {
      datoCuentaBancaria.push({
        idProveedor: this.idProveedor,
        idProveedorCuentaBancaria:
          this.array_cuentasBancarias[i].idProveedorCuentaBancaria,
    ...
    for (let i = 0; i < this.array_cuentaBancaria_eliminado.length; i++) {
      datoCuentaBancaria.push({
        idProveedor: this.idProveedor,
        idProveedorCuentaBancaria:
          this.array_cuentaBancaria_eliminado[i].idProveedorCuentaBancaria,
    ...
    return {
      ...
      ProveedorContacto: datoContacto,
      ProveedorCuentaBancaria: datoCuentaBancaria,
    };
  }
