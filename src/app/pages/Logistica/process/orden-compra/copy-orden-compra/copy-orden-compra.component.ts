...
@Component({
  selector: "app-copy-orden-compra",
  templateUrl: "./copy-orden-compra.component.html",
  styleUrls: ["./copy-orden-compra.component.scss"],
  providers: [
    DatePipe,
...
export class CopyOrdenCompraComponent implements OnInit {
  /* Datos generales */
    ...
    this.orden_compra_s.GetDatosOrdenCompra(idOrdenCompra).subscribe(
      (data: any) => {
        ...
        this.getDatosProveedor(
          datos_generales.Proveedor,
          datos_generales.IdCuentaBancaria,
          datos_generales.IdContacto
        );
        this.getUltimasCompras(datos_generales.Proveedor, idOrdenCompra);
        this.getProveedores(datos_generales.Proveedor);
        this.controlsDatos.CodigoCotizacionProv.setValue(
          datos_generales.CodigoCotizacion
        );
        this.getTipoDocumentos(datos_generales.IdTipoDocumento);
        this.getFormaPago(datos_generales.IdFormaPago);
        ...
        this.getTipoMoneda(datos_generales.idMoneda);
        this.controlsDatos.TasaCambio.setValue(
          datos_generales.TasaCambio.toFixed(3)
        );
        this.getEstablecimientos(datos_generales.IdEstablecimiento);
  ...
  getTipoMoneda(value) {
    this.multitabla_s.GetListarMoneda().subscribe(
      (data: any) => {
        ...
        if (value != "" && value != null) {
          this.controlsDatos.TipoMoneda.setValue(value);
        }
  ...
  changeFechaEmision() {
    ...
    for (let i = 0; i < this.array_docsCompra.length; i++) {
      ...
      if (fechaDc < fechaEmision) {
        dcctrl.FechaEmision.setValue(this.controlsDatos["FechaEmision"].value);
      }
    }
    this.changeTipoCambio(1, 0);
  }
