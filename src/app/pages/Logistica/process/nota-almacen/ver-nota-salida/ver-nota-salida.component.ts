...
@Component({
  selector: "app-ver-nota-salida",
  templateUrl: "./ver-nota-salida.component.html",
  styleUrls: ["./ver-nota-salida.component.scss"],
  providers: [
    DatePipe,
...
export class VerNotaSalidaComponent implements OnInit {
  @ViewChild('pedidoVenta') pedidoVenta:any;
  ...
  mostrarOS: boolean = true;

  /* Datos OTM */
  mostrarOTM: boolean = true;

  /* Datos Pedido Venta */
  mostrarPV: boolean = false;
  ...
  getDatosNotaAlmacen(idNotaAlmacen) {
    this.nota_almacen_s.GetDatosNotaAlmacen(idNotaAlmacen).subscribe(
      (data: any) => {
        ...
        if (datosG.idTipoDocumento == "08") {
          this.mostrarOtros = false;
          this.mostrarPV = true;
          this.mostrarBotones = true;
          this.pedidoVenta.getDatosPV(datosG.idPedidoVenta, data);
        } else {
          this.controlsDatos.Observacion.setValue(datosG.observaciones);
