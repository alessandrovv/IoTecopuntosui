...
@Component({
  selector: "app-save-pedido-venta",
  templateUrl: "./save-pedido-venta.component.html",
  styleUrls: ["./save-pedido-venta.component.scss"],
  providers: [
    DatePipe,
...
export class SavePedidoVentaComponent implements OnInit {
  /* Datos Generales */
  ...
  formularioDatosGenerales() {
    this.formDatosGenerales = this.fb.group({
      ...
      ClienteSucursal: [null, Validators.compose([Validators.required])],
  ...
  /* CLIENTE */
  nuevoCliente() {
    const modalRef = this.modalService.open(ClienteModalComponent,{size: "xl"});
  ...
  nuevaSucursal() {
    if (this.controlsDatos.Cliente.invalid) {
      this.toastr.warningToastr('No has seleccionado un cliente.', "Advertencia!", {
        toastTimeout: 2000,
      ...
      const modalRef = this.modalService.open(SaveUpdateSucursalComponent, {size: "md"});
      ...
      modalRef.result.then(
        (result:any) => {
          this.getSucursales(-1);
          this.getClienteSucursal(this.controlsDatos.Cliente.value);
        }, (reason:any) => {
          console.log(reason);
        }
      );
  ...
  agregarMaterial(material) {
    ...
    if (!existe) {
      ...
      const materialForm = this.fb.group({
        ...
        Descripcion: [newMaterial.nombreMaterial],
        CantidadTotal: [newMaterial.CantidadTotal],
        ValorUnit: [
          {
            value: parseFloat(newMaterial.valorUnit).toFixed(
        ...
        DescuentoUnit: [
          parseFloat(newMaterial.descuentoUnit).toFixed(
            this.controlsDatos.NumeroDecimales.value
          ),
          Validators.compose([Validators.required]),
        ],
        IgvUnit: [parseFloat(newMaterial.igvUnit).toFixed(6)],
        PrecioUnit: [
          {
            value: parseFloat(newMaterial.precioUnit).toFixed(
              this.controlsDatos.NumeroDecimales.value
            ),
        ...
        PrecioTotal: [
          {
            value: parseFloat(newMaterial.precioTotal).toFixed(
              this.controlsDatos.NumeroDecimales.value
            ),
            disabled: true,
          },
      ...
      this.array_materiales.push(newMaterial);
      this.sumarMontos();
      this.chgRef.markForCheck();
    } else {
    ...
    if (cliente != null) {
      this.pedido_venta_s.GetDataCliente(cliente).subscribe(
        (data: any) => {
          ...
          let datosg = data[2][0];
          if (datosg) {
            this.controlsDatos.MontoHistorico.setValue(datosg.montoHistorico);
            this.controlsDatos.FechaUltimaVenta.setValue(
              this.datePipe.transform(datosg.fechaUltimaVenta, "yyyy-MM-dd")
            );            
          }
          this.chgRef.markForCheck();
        },
