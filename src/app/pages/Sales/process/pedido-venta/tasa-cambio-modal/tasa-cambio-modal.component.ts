...
@Component({
  selector: "app-tasa-cambio-modal",
  templateUrl: "./tasa-cambio-modal.component.html",
  styleUrls: ["./tasa-cambio-modal.component.scss"],
  providers: [
    DatePipe,
...
export class TasaCambioModalComponent implements OnInit {
  @Input() fecha: any;
  ...
  save() {
    ...
    this.pedido_venta_s.SaveUpdateTasaCambio(datos).subscribe(
      (data: any) => {
        if (data[0].Ok === 1) {
          this.toastr.successToastr(data[0].Message, "Operacion exitosa!", {
            toastTimeout: 1500,
